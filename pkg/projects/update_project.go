...
func (h handler) UpdateProject(c *gin.Context) {
	...
	if result := h.DB.Raw(`update projetos set nome_projeto = ?, equipe_id = ?, status = ?, descricao_projeto = ? where id_projeto = ?`, projeto.Nome_Projeto, projeto.EquipeID, projeto.Status, projeto.Descricao_Projeto, id).Scan(&projeto); result.Error != nil {
